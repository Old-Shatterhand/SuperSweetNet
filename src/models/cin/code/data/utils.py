...
"""def get_rings(edge_index, max_k=7):
    ...
    edge_list = edge_index.T
    graph_gt = nx.Graph()
    graph_gt.add_edges_from(edge_list)
    ...
    for k in range(3, max_k + 1):
        ...
        pattern_edge_list = list(pattern.edges)
        pattern_gt = nx.Graph()
        pattern_gt.add_edges_from(pattern_edge_list)
        sub_isos = subgraph_isomorphism(pattern_gt, graph_gt, induced=True, subgraph=True, generator=True)
        sub_iso_sets = map(lambda isom: tuple(isom.keys()), sub_isos)
        for iso in sub_iso_sets:
            if tuple(sorted(iso)) not in sorted_rings:
                rings.add(iso)
                sorted_rings.add(tuple(sorted(iso)))
...
def get_rings(edge_index, max_k=7):
    ...
    rings = list(sorted([tuple(sorted(x)) for x in nx.cycle_basis(graph_gt)], key=lambda x: (len(x), x[0])))
    # print(len(rings))
    return rings
...
def subgraph_isomorphism(sub, g, max_n=0, vertex_label=None, edge_label=None,
    ...
    matcher = isomorphism.GraphMatcher(sub, g)
...
def convert_graph_dataset_with_rings(dataset, max_ring_size=7, include_down_adj=False,
    ...
    complexes = [compute_ring_2complex(
